...
class TodoController {
    ...
    async getTodosAndComments(req: Request, res: Response) {
        // const userPayload = await authService.getTokenPayload(req, 'ACCESS');
        const params = validate(req, todoIdSchema, 'params');

        const todos = await todoService.getTodoAndComments(params);

        sendResponse(res, {
            statusCode: StatusCodes.CREATED,
            success: true,
            message: 'Successfully found a todo and its comments',
            data: { todos }
        });
    }
